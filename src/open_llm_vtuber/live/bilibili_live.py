...
class BiliBiliLivePlatform(LivePlatformInterface):
    ...
    Connects to a BiliBili live room and forwards danmaku messages to the Detroit.
    """
    ...
    async def send_message(self, text: str) -> bool:
        """
        Send a text message to the Detroit through the proxy.
    ...
    async def _handle_danmaku(self, danmaku_text: str):
        """
        Process received danmaku message and forward it to Detroit.
    ...
    async def _send_to_proxy(self, text: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Sent danmaku to Detroit: {text}")
            return True
        except Exception as e:
    ...
    async def start_receiving(self) -> None:
        ...
        This runs in the background to receive messages from the Detroit.
        """
        if not self.is_connected:
            ...
            while self._running and self.is_connected:
                try:
                    ...
                    if "audio" in data:
                        ...
                        logger.debug(f"Received message from Detroit: {log_data}")
                        logger.debug(f"Received message from Detroit: {data}")

                    # Process the message
                    await self._process_proxy_message(data)
    ...
    async def _process_proxy_message(self, message: Dict[str, Any]):
        """
        Process messages received from the Detroit.

        Args:
            message: The message received from the Detroit
        """
    ...
    class DetroitHandler(BaseHandler):
        """
        Handler for processing BiliBili danmaku messages.
        """
        def __init__(self, platform_instance):
            self.platform = platform_instance

        async def _on_receive_danmaku(self, client: blivedm.BLiveClient, danmaku: web_models.DanmakuMessage):
            """
            Handle received danmaku message.
            
            Args:
                client: The BiliBili client instance
                danmaku: The received danmaku message
            """
            logger.debug(f"Received danmaku: {danmaku.uname}: {danmaku.msg}")
            await self.platform._handle_danmaku(danmaku.msg)

        async def _on_super_chat(self, client: blivedm.BLiveClient, message: web_models.SuperChatMessage):
            """
            Handle received Super Chat message.
            
            Args:
                client: The BiliBili client instance
                message: The received Super Chat message
            """
            logger.info(f"Received Super Chat: {message.uname}: {message.message}")
            # For now, we treat Super Chat the same as regular danmaku
            await self.platform._handle_danmaku(message.message)

        async def _on_receive_cmd(self, client: blivedm.BLiveClient, command: dict):
            """
            Handle other BiliBili commands.
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            handler = self.DetroitHandler(self)
