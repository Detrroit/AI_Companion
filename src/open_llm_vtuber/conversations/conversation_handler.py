...
async def handle_conversation_trigger(
    ...
    # Remove image data to prevent it from being sent to LLM
    # This disconnects the webcam from the LLM while preserving it for future use
    images = None  # Always set to None to prevent image data from being sent to LLM
    ...
    if group and len(group.members) > 1:
        ...
        if (
            ...
            or (current_conversation_tasks[task_key] is not None and current_conversation_tasks[task_key].done())
        ):
            ...
            current_conversation_tasks[task_key] = asyncio.create_task(
                process_group_conversation(
                    ...
                    images=images,  # Pass None to prevent image data from being sent
...
async def handle_group_interrupt(
    ...
    await broadcast_to_group(
        list(group.members) if group else [],
        {
