...
class BasicMemoryAgent(AgentInterface):
    ...
    def _to_text_prompt(self, input_data: BatchInput) -> str:
        ...
        # Remove image references from text prompt to prevent LLM from trying to process them
    ...
    def _to_messages(self, input_data: BatchInput) -> List[Dict[str, Any]]:
        ...
        # Remove image processing to prevent LLM from trying to process image data
        # This disconnects the webcam from the LLM while preserving it for future use
        # if input_data.images:
        #     image_added = False
        ...
        if user_content:
            ...
            if not skip_memory:
                self._add_message(
                    text_prompt if text_prompt else "[User provided text input]", "user"
                )
        else:
